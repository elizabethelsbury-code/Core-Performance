...
let sessions = [];
let goals = [];
let settings = { startDate: DEFAULT_START_DATE, completeAcknowledged: false, adaptiveDeloadStart: null, autoReportEnabled: false, lastBackupAt: null };
let PLAN = {};
let editingProgram = false;
let editorMode = 'live'; // 'live' or 'template'
...
function finishEditingPlan(main){
  if(editorMode === 'template'){
    saveTemplates();
    editorMode = 'live'; editingTemplateId = null;
...
// Reads the leading set count off a target string like '3x8-10' or
// '3-4x4-6' (lower bound for a range) so the Log view can start with that
// many empty set rows already there, instead of an empty list every time.
function initialSetCountForTarget(target){
  if(!target) return 0;
  const m = /^(\d+)(?:[–-]\d+)?\s*x/i.exec(target.trim());
  if(!m) return 0;
...
function emptyExercisesFor(dayKey){
  ...
  return (day.exercises||[]).map(e => {
    const setCount = initialSetCountForTarget(e.target);
    const sets = [];
    for(let i=0; i<setCount; i++) sets.push({ id: uid(), weight:'', reps:'', rir:'', note:'' });
    return { id: uid(), name: e.name, originalName: e.name, target: e.target, hero: !!e.hero, custom:false, sets };
  });
}
...
let draftSaveDebounceTimer = null;
function debouncedSaveDraft(){
  if(view !== 'log') return;
  clearTimeout(draftSaveDebounceTimer);
  draftSaveDebounceTimer = setTimeout(saveDraft, 400);
}
function loadDraft(){
  try{
...
// If the app is left open/backgrounded across midnight, several "today"
// defaults were only ever set once at load and would otherwise stay stuck on
// the old day. On resume, refresh anything still sitting on the stale date —
// but never touch a date the user deliberately navigated to, or a session
// they're mid-edit on.
let lastKnownToday = todayISO();
function refreshForNewDayIfNeeded(){
  const now = todayISO();
  if(now === lastKnownToday) return;
  const oldToday = lastKnownToday;
  lastKnownToday = now;
  if(!editingId && logState.date === oldToday){
    logState.date = now;
    resetLogForDay(weekdayForISO(now));
  }
  if(checkinDate === oldToday) checkinDate = now;
  if(cycleState.date === oldToday) cycleState.date = now;
  if(measureDraft && measureDraft.date === oldToday) measureDraft.date = now;
  if(calendarCursor === ymOf(oldToday)) calendarCursor = ymOf(now);
  renderAll();
}
document.addEventListener('visibilitychange', ()=>{ if(document.visibilityState === 'visible') refreshForNewDayIfNeeded(); });
window.addEventListener('focus', refreshForNewDayIfNeeded);
window.addEventListener('pageshow', refreshForNewDayIfNeeded);
...
function renderNav(){
  ...
  document.querySelectorAll('.navbtn').forEach(b=>{
    b.onclick = ()=>{
      saveDraft(); // immediate, synchronous save the instant you switch tabs —
                    // don't rely on the debounce timer catching up in time
      view = b.dataset.view;
      renderAll();
    };
  });
...
// ---------- CHECK-IN (readiness + habits) ----------
let checkinDate = todayISO();
function renderCheckin(main){
  const main2 = main;
  let html = `<div class="field" style="margin-bottom:14px;"><label>Date</label>
    <div style="overflow:hidden;border-radius:7px;"><input type="date" id="checkinDateInput" value="${checkinDate}" style="border-radius:7px;"></div></div>`;
  html += renderReadinessCard(checkinDate);
  html += renderHabitsCard(checkinDate);
  main.innerHTML = html;
  document.getElementById('checkinDateInput').onchange = e=>{ checkinDate = e.target.value; renderCheckin(main2); };
  wireReadiness(main, checkinDate, ()=>renderCheckin(main2));
  wireHabits(main, checkinDate, ()=>renderCheckin(main2));
}
...
function setReadinessRating(dateISO, field, value){
  ...
  existing[field] = (existing[field] === value) ? 0 : value; // tap the selected value again to clear it
  saveReadinessEntry(existing);
  showToast(existing[field] ? `${field.charAt(0).toUpperCase()+field.slice(1)} saved` : `${field.charAt(0).toUpperCase()+field.slice(1)} cleared`);
}
...
function wireReadiness(main, dateISO, rerender){
  main.querySelectorAll('.readinessChip').forEach(btn=>{
    btn.onclick = ()=>{
      setReadinessRating(dateISO, btn.dataset.field, parseInt(btn.dataset.val,10));
      rerender();
    };
  });
}
...
function renderHabitManageOverlay(main, rerender){
  ...
  document.getElementById('closeHabitManageBtn').onclick = ()=>{ ov.style.display='none'; rerender(); };
  document.getElementById('addHabitBtn').onclick = ()=>{
    ...
    saveHabits();
    habitsExpanded = true;
    renderHabitManageOverlay(main, rerender);
  };
...
function openHabitManage(main, rerender){
  const ov = ensureHabitManageOverlay();
  renderHabitManageOverlay(main, rerender);
  ov.style.display = 'flex';
}
function wireHabits(main, dateISO, rerender){
  main.querySelectorAll('.habitChip').forEach(btn=>{
    btn.onclick = ()=>{
      const h = habits.find(x=>x.id===btn.dataset.id);
      if(h){ toggleHabitDone(h, dateISO); rerender(); }
    };
  });
  const toggleBtn = document.getElementById('habitsToggleBtn');
  if(toggleBtn) toggleBtn.onclick = ()=>{ habitsExpanded = !habitsExpanded; rerender(); };
  const manageBtn = document.getElementById('manageHabitsBtn');
  if(manageBtn) manageBtn.onclick = ()=>openHabitManage(main, rerender);
}
...
function renderMeasure(main){
  ...
  html += `<div class="card" style="margin-bottom:14px;">
    ...
    <div class="field" style="margin-bottom:10px;"><label>Date</label><div style="overflow:hidden;border-radius:7px;"><input type="date" id="measureDate" value="${d.date}" style="border-radius:7px;"></div></div>
...
function renderCycle(main){
  ...
  let html = `<div class="field" style="margin-bottom:8px;"><label>Date</label><div style="overflow:hidden;border-radius:7px;"><input type="date" id="cycleDateInput" value="${cycleState.date}" style="border-radius:7px;"></div></div>
...
function renderLog(main){
  // Self-heal a stale/mismatched day: if we're looking at today's date but
  // logState.dayKey doesn't match today's actual weekday, and nothing has
  // been entered yet (so there's genuinely nothing to lose), correct it
  // automatically instead of silently showing the wrong Program day.
  if(!editingId && logState.date === todayISO() && logState.dayKey !== todayDayName()){
    const nothingEntered = logState.exercises.every(ex => ex.sets.every(s => !s.weight && !s.reps && !s.note));
    if(nothingEntered){
  ...
  html += `<div class="field" style="margin-bottom:10px;"><label>Date</label><div style="overflow:hidden;border-radius:7px;"><input type="date" id="logDate" value="${logState.date}" style="border-radius:7px;"></div></div>
  <div class="field" style="margin-bottom:14px;"><label>Day</label>
  ...
  if(day.type === 'lift'){
    ...
    html += `<button class="pill" id="addExerciseBtn" style="width:100%;padding:12px;margin-bottom:10px;border-style:dashed;">+ Add exercise</button>`;
    if(editingId){
      html += `<button class="pill" id="addRemainingBtn" style="width:100%;padding:12px;margin-bottom:14px;border-style:dashed;">+ Add remaining exercises from today's plan</button>`;
    } else {
      html += `<button class="pill" id="copyLastWorkoutBtn" style="width:100%;padding:12px;margin-bottom:14px;border-style:dashed;">&#128203; Copy last ${escapeHtml(logState.dayKey)}'s workout</button>`;
    }
  }
  ...
  if(logState.cardio.active){
    html += `<div class="card">
      <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
        ...
        <button class="pill ${logState.cardio.type==='Bike'?'active':''}" data-cardio-type="Bike">BIKE</button>
      </div>
      <div class="row2">
  ...
  document.getElementById('logDay').onchange = e=>{ resetLogForDay(e.target.value); renderLog(main); };
  document.getElementById('logWarmup').oninput = e=>{ logState.warmup = e.target.value; debouncedSaveDraft(); };
  document.getElementById('logDuration').oninput = e=>{ logState.duration = e.target.value; debouncedSaveDraft(); };
  const noteEl = document.getElementById('logNote'); if(noteEl) noteEl.oninput = e=>{ logState.note = e.target.value; debouncedSaveDraft(); };
  const addExBtn = document.getElementById('addExerciseBtn');
  if(addExBtn) addExBtn.onclick = ()=>{ logState.exercises.push({id:uid(), name:'', originalName:'', target:'', hero:false, custom:true, sets:[]}); renderLog(main); };
  const addRemainingBtn = document.getElementById('addRemainingBtn');
  if(addRemainingBtn) addRemainingBtn.onclick = ()=>openAddRemainingPicker(main);
  const copyLastBtn = document.getElementById('copyLastWorkoutBtn');
  if(copyLastBtn) copyLastBtn.onclick = ()=>{
    const hasData = logState.exercises.some(ex=>ex.sets.some(s=>s.weight||s.reps||s.note));
    if(hasData && !confirm("Replace your current exercise list with last time's workout for this day? Anything already entered here will be lost.")) return;
    const copiedDate = copyLastWorkoutForDay();
    if(!copiedDate){ showToast('No previous session found for this day'); return; }
    showToast(`Copied from ${formatAU(copiedDate)}`);
    renderLog(main);
  };
  ...
  const cd = document.getElementById('cardioDist'); if(cd) cd.oninput = e=>{ logState.cardio.distance = e.target.value; debouncedSaveDraft(); };
  const ct = document.getElementById('cardioTime'); if(ct) ct.oninput = e=>{ logState.cardio.time = e.target.value; debouncedSaveDraft(); };
  const cn = document.getElementById('cardioNote'); if(cn) cn.oninput = e=>{ logState.cardio.note = e.target.value; debouncedSaveDraft(); };
...
// ---------- LINKED EXERCISE NAMES (same slot, different equipment/name) ----------
const EXERCISE_GROUPS_KEY = 'core-performance-exercise-groups';
let exerciseGroups = [];
function loadExerciseGroups(){
  try{
    const raw = localStorage.getItem(EXERCISE_GROUPS_KEY);
    if(raw) exerciseGroups = JSON.parse(raw);
  }catch(e){ exerciseGroups = []; }
}
function saveExerciseGroups(){
  try{ localStorage.setItem(EXERCISE_GROUPS_KEY, JSON.stringify(exerciseGroups)); }
  catch(e){ showToast("Couldn't save exercise link"); }
}
function linkExerciseNames(names){
  if(!names || names.length < 2) return;
  const keys = names.map(canonicalKey);
  let target = exerciseGroups.find(g=>g.names.some(n=>keys.includes(canonicalKey(n))));
  if(target){
    const existingKeys = new Set(target.names.map(canonicalKey));
    names.forEach(n=>{ if(!existingKeys.has(canonicalKey(n))) target.names.push(n); });
  } else {
    exerciseGroups.push({ id: uid(), names: [...names] });
  }
  saveExerciseGroups();
}
function groupNamesFor(name){
  const key = canonicalKey(name);
  const group = exerciseGroups.find(g=>g.names.some(n=>canonicalKey(n)===key));
  return group ? group.names : [name];
}

function getLastLoggedExercise(name, includeDeloadWeeks){
  const sorted = [...sessions].sort((a,b)=>b.date.localeCompare(a.date));
  const groupKeys = new Set(groupNamesFor(name).map(canonicalKey));
  for(const s of sorted){
    if(!includeDeloadWeeks && getPhaseForDate(s.date).deload) continue;
    const ex = (s.exercises||[]).find(e=>groupKeys.has(canonicalKey(e.name)));
    if(ex && ex.sets && ex.sets.length){
      return { date: s.date, sets: ex.sets, matchedName: ex.name };
    }
  }
  // If every logged instance happens to be from a deload week (e.g. this is
  // brand new), fall back to including them rather than showing nothing.
  if(!includeDeloadWeeks) return getLastLoggedExercise(name, true);
  return null;
}
function lastTimeReference(name){
  ...
  const viaName = (last.matchedName && canonicalKey(last.matchedName) !== canonicalKey(name)) ? last.matchedName : null;
  return { date: last.date, text, viaName };
}

function renderExerciseCard(ex){
  const idx = logState.exercises.findIndex(x=>x.id===ex.id);
  const isFirst = idx <= 0;
  const isLast = idx === logState.exercises.length - 1;
  ...
  if(!ex.custom && ex.target){
    ...
    if(getPhaseForDate(logState.date).deload){
      ...
      const dw = computeDeloadWeightSuggestion(ex.name);
      if(dl) html += `<div style="font-family:var(--font-mono);font-size:10px;color:var(--steel);margin-top:3px;">Deload: ${escapeHtml(dl)}${dw!==null?` — try ${fmtWeight(dw)}`:''}</div>`;
    }
  }
  ...
  html += `</div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <button class="moveExUpBtn" data-ex-id="${ex.id}" ${isFirst?'disabled':''} style="background:none;border:none;color:${isFirst?'var(--border)':'var(--text-faint)'};font-size:13px;padding:2px 6px;">&#9650;</button>
      <button class="moveExDownBtn" data-ex-id="${ex.id}" ${isLast?'disabled':''} style="background:none;border:none;color:${isLast?'var(--border)':'var(--text-faint)'};font-size:13px;padding:2px 6px;">&#9660;</button>
    </div>
    <button class="removeExBtn" data-ex-id="${ex.id}" style="background:none;border:none;color:var(--text-faint);font-size:16px;">&times;</button>
  </div>`;

  if(ex.name && ex.name.trim()){
    const lastTime = lastTimeReference(ex.name);
    if(lastTime){
      const viaNote = lastTime.viaName ? ` <span style="text-transform:none;">(as "${escapeHtml(lastTime.viaName)}")</span>` : '';
      html += `<div style="background:var(--surface-2);border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:10px;font-size:12px;color:var(--brass);line-height:1.4;">
        <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-faint);text-transform:uppercase;letter-spacing:0.04em;">Last time · ${formatAU(lastTime.date)}${viaNote}</span><br>
  ...
  ex.sets.forEach((s,i)=>{
    html += `<div class="setrow" data-ex-id="${ex.id}" data-set-id="${s.id}">
      ...
      <select class="set-field" data-field="rir">
        ...
        ${['0-1','1-2','3-4','Failure'].map(r=>`<option value="${r}" ${s.rir===r?'selected':''}>${r}</option>`).join('')}
      </select>
...
function wireExerciseCards(main){
  let exNameRerenderTimer = null;
  main.querySelectorAll('.ex-name-input').forEach(inp=>{
    inp.oninput = e=>{
      const ex = logState.exercises.find(x=>x.id===inp.dataset.exId);
      if(ex){ ex.name = e.target.value; debouncedSaveDraft(); }
      clearTimeout(exNameRerenderTimer);
      exNameRerenderTimer = setTimeout(()=>renderLog(main), 900);
    };
    // Once she's done editing the name (taps away), refresh the card so "Last
    // time", target/deload badges, and the warm-up calc button all recompute
    // against the new name — otherwise they keep showing the old exercise's
    // data since they were only rendered once, at the old name.
    inp.onblur = ()=>{ clearTimeout(exNameRerenderTimer); renderLog(main); };
  });
  ...
  main.querySelectorAll('.moveExUpBtn').forEach(btn=>{
    btn.onclick = ()=>{
      const i = logState.exercises.findIndex(x=>x.id===btn.dataset.exId);
      if(i > 0){
        [logState.exercises[i-1], logState.exercises[i]] = [logState.exercises[i], logState.exercises[i-1]];
        debouncedSaveDraft();
        renderLog(main);
      }
    };
  });
  main.querySelectorAll('.moveExDownBtn').forEach(btn=>{
    btn.onclick = ()=>{
      const i = logState.exercises.findIndex(x=>x.id===btn.dataset.exId);
      if(i >= 0 && i < logState.exercises.length-1){
        [logState.exercises[i], logState.exercises[i+1]] = [logState.exercises[i+1], logState.exercises[i]];
        debouncedSaveDraft();
        renderLog(main);
      }
    };
  ...
  main.querySelectorAll('.setrow').forEach(row=>{
    ...
    row.querySelectorAll('.set-field').forEach(f=>{
      ...
      f[ev] = e=>{
        ...
        if(s){ s[f.dataset.field] = e.target.value; debouncedSaveDraft(); }
      };
    });
...
function ensureNotificationPermission(){
  if(!('Notification' in window)){
    showToast('This browser doesn\'t support notifications — you\'ll still get the in-app beep', 3000);
    return;
  }
  if(Notification.permission === 'default'){
    Notification.requestPermission().then(perm=>{
      if(perm === 'granted'){
        showToast('Notifications on — you\'ll get an alert when rest ends', 2200);
      } else {
        showToast('Notifications off — you\'ll only hear the beep while the app is open', 3200);
      }
    }).catch(()=>{});
  } else if(Notification.permission === 'denied'){
    showToast('Notifications are blocked for this app in your phone settings', 3200);
  }
}
function showRestNotification(){
  ...
  try{
    new Notification("Rest's up!", { body: 'Time to get back to it.' });
  }catch(e){}
}
...
function unlockRestAudio(){
  try{
    ...
    // play a near-silent blip right now, synchronously within the tap, to fully unlock iOS audio
    ...
    gain.gain.setValueAtTime(0.0001, restAudioCtx.currentTime);
    osc.start(restAudioCtx.currentTime);
    osc.stop(restAudioCtx.currentTime + 0.05);
  }catch(e){}
}
function playRestTimerBeep(){
  try{
    ...
    [0, 0.6, 1.2].forEach(delay=>{
      ...
      gain.gain.setValueAtTime(0.18, restAudioCtx.currentTime + delay);
      osc.start(restAudioCtx.currentTime + delay);
      osc.stop(restAudioCtx.currentTime + delay + 0.35);
    });
  }catch(e){}
...
let restTimerAlarmTimeout = null;
// ---------- REST TIMER BACKGROUND PUSH (fires even fully backgrounded) ----------
// Fill in REST_TIMER_WORKER_URL after deploying the Cloudflare Worker in
// /worker (see worker/README.md for exact steps). Leave it blank and
// everything falls back to the in-app-only beep/notification as before.
const REST_TIMER_WORKER_URL = ''; // e.g. 'https://core-performance-rest-timer.yourname.workers.dev'
const VAPID_PUBLIC_KEY = 'BPCkTPWWXb7XOXr0sroZ9nNve4XbogHrrEycPFRWThes6aPdEJvbdGsgnJC1TYBze287hwWUl8Ohuf38x82sA_s';
...
async function registerPushServiceWorker(){
  if(!('serviceWorker' in navigator) || !REST_TIMER_WORKER_URL) return;
  try{
    const reg = await navigator.serviceWorker.register('sw.js');
    await navigator.serviceWorker.ready;
    if(Notification.permission === 'granted'){
      pushSubscription = await ensurePushSubscription(reg);
    }
  }catch(e){}
}
...
async function scheduleBackgroundRestAlarm(seconds){
  if(!REST_TIMER_WORKER_URL) return null;
  try{
    const reg = await navigator.serviceWorker.ready;
    pushSubscription = pushSubscription || await ensurePushSubscription(reg);
    if(!pushSubscription) return null;
    const res = await fetch(REST_TIMER_WORKER_URL + '/schedule', {
      method: 'POST',
...
async function cancelBackgroundRestAlarm(id){
  if(!REST_TIMER_WORKER_URL || !id) return;
  try{
    await fetch(REST_TIMER_WORKER_URL + '/cancel', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function startRestTimer(seconds){
  ...
  saveRestTimerState({ endTime: Date.now() + seconds*1000, beeped:false, pushAlarmId:null });
  renderRestTimerBar();
  if(restTimerAlarmTimeout) clearTimeout(restTimerAlarmTimeout);
  restTimerAlarmTimeout = setTimeout(()=>{
    const state = loadRestTimerState();
    if(state && !state.beeped){
      state.beeped = true;
      saveRestTimerState(state);
      playRestTimerBeep();
    }
  }, seconds*1000);
  scheduleBackgroundRestAlarm(seconds).then(id=>{
    if(!id) return;
    const state = loadRestTimerState();
    if(state){ state.pushAlarmId = id; saveRestTimerState(state); }
  });
}
function adjustRestTimer(deltaSeconds){
  ...
  state.beeped = false;
  const oldAlarmId = state.pushAlarmId;
  state.pushAlarmId = null;
  saveRestTimerState(state);
  restTimerTick();
  if(restTimerAlarmTimeout) clearTimeout(restTimerAlarmTimeout);
  const remainingMs = Math.max(0, state.endTime - Date.now());
  restTimerAlarmTimeout = setTimeout(()=>{
    const s = loadRestTimerState();
    if(s && !s.beeped){
      s.beeped = true;
      saveRestTimerState(s);
      playRestTimerBeep();
    }
  }, remainingMs);
  if(oldAlarmId) cancelBackgroundRestAlarm(oldAlarmId);
  scheduleBackgroundRestAlarm(Math.round(remainingMs/1000)).then(id=>{
    if(!id) return;
    const s = loadRestTimerState();
    if(s){ s.pushAlarmId = id; saveRestTimerState(s); }
  });
}
function stopRestTimer(){
  const state = loadRestTimerState();
  if(state && state.pushAlarmId) cancelBackgroundRestAlarm(state.pushAlarmId);
  saveRestTimerState(null);
  if(restTimerAlarmTimeout){ clearTimeout(restTimerAlarmTimeout); restTimerAlarmTimeout = null; }
  renderRestTimerBar();
}
...
function renderRestTimerBar(){
  ...
  if(state){
    ...
    bar.querySelectorAll('.restPresetBtn').forEach(b=>{
      b.onclick = ()=>{ unlockRestAudio(); startRestTimer(parseInt(b.dataset.s,10)); };
    });
  }
}
...
// Rounds a suggested weight to what's actually available. Dumbbells only come
// in 2.5kg jumps starting at 5kg (both gyms). Everything else (barbell, cable,
// machine) also rounds to 2.5kg — since barbell plates load symmetrically in
// pairs, the finest real total-weight increment with 1.25kg plates is 2.5kg,
// not 1.25kg, and this also avoids missing exercises like "Conventional
// deadlift" that are barbell by default but don't say so in the name.
function isDumbbellExercise(name){
  if(!name) return false;
  return /\bdb\b|dumbbell/i.test(name);
}
function roundToAvailableWeight(weight, exerciseName){
  if(weight <= 0) return 0;
  if(isDumbbellExercise(exerciseName)){
    return Math.max(5, Math.round(weight/2.5)*2.5);
  }
  return Math.round(weight/2.5)*2.5;
}
// Computes an actual deload weight in kg from the exercise's last logged top
// set, so the ~65% note isn't just a percentage she has to calculate herself.
function computeDeloadWeightSuggestion(exerciseName){
  const last = getLastLoggedExercise(exerciseName);
  if(!last) return null;
  const top = topWeight(last);
  if(top === null) return null;
  return roundToAvailableWeight(top*0.65, exerciseName);
}
function computeWarmupSets(workingWeight, exerciseName){
  const steps = [ {pct:0.4, reps:8}, {pct:0.6, reps:5}, {pct:0.8, reps:3} ];
  return steps.map(s=>({
    weight: roundToAvailableWeight(workingWeight*s.pct, exerciseName),
    reps: s.reps
  }));
}
...
function renderWarmupResults(){
  ...
  const exName = document.getElementById('warmupExName').textContent;
  const sets = computeWarmupSets(w, exName);
...
function computeNewPRs(loggedExercises, excludeSessionId){
  ...
  loggedExercises.forEach(ex=>{
    ...
    let priorBestVolume = 0; // best-ever total volume (weight×reps summed) in a single session
    const priorBestRepsAtWeight = new Map(); // weight -> most reps ever done at that exact weight
    sessions.forEach(s=>{
      ...
      (s.exercises||[]).forEach(e=>{
        ...
        (e.sets||[]).forEach(st=>{
          ...
          if(!isNaN(w) && w > priorBestWeight) priorBestWeight = w;
          if(!isNaN(w) && !isNaN(r)){
            const prevReps = priorBestRepsAtWeight.get(w) || 0;
            if(r > prevReps) priorBestRepsAtWeight.set(w, r);
            sessionVolume += w*r;
          }
        });
        if(sessionVolume > priorBestVolume) priorBestVolume = sessionVolume;
      });
    });
    ...
    const todaysVolume = ex.sets.reduce((sum,s)=>{
      const w = parseFloat(s.weight), r = parseInt(s.reps, 10);
      return (!isNaN(w) && !isNaN(r)) ? sum + w*r : sum;
    }, 0);
    if(todaysBest > 0 && todaysBest > priorBestWeight){
      prs.push({ name: ex.name, weight: todaysBest, type: 'weight' });
    } else {
      // No new weight PR this exercise — check for a rep PR: matching a weight
      // you've lifted before, but for more reps than you've ever done at it.
      let repPR = null;
      ex.sets.forEach(s=>{
        const w = parseFloat(s.weight);
        ...
        const prevReps = priorBestRepsAtWeight.get(w) || 0;
        if(prevReps > 0 && r > prevReps && (!repPR || r > repPR.reps)){
          repPR = { weight: w, reps: r, prevReps };
        }
      });
      if(repPR){
        prs.push({ name: ex.name, weight: repPR.weight, reps: repPR.reps, prevReps: repPR.prevReps, type: 'reps' });
      } else if(todaysVolume > 0 && priorBestVolume > 0 && todaysVolume > priorBestVolume){
        // No weight or rep PR either — but if this session's total volume for
        // this exercise beats your best-ever single session, that's still
        // real progress (e.g. an extra set), worth catching on its own.
        prs.push({ name: ex.name, volume: Math.round(todaysVolume), prevVolume: Math.round(priorBestVolume), type: 'volume' });
      }
    }
  });
  return prs;
...
function handleSave(){
  ...
  if(newPRs.length){
    const parts = newPRs.map(p=>{
      if(p.type === 'reps') return `${p.name} ${fmtWeight(p.weight)}×${p.reps} (was ×${p.prevReps})`;
      if(p.type === 'volume') return `${p.name} ${p.volume}kg total (was ${p.prevVolume}kg)`;
      return `${p.name} ${fmtWeight(p.weight)}`;
    });
    const hasWeightPR = newPRs.some(p=>p.type==='weight');
    const hasRepPR = newPRs.some(p=>p.type==='reps');
    const icon = hasWeightPR ? '🏆' : hasRepPR ? '💪' : '📈';
    const label = hasWeightPR ? 'New PR!' : hasRepPR ? 'Rep PR!' : 'Volume PR!';
    toastMsg = `${icon} ${label} ${parts.join(', ')}`;
    toastDuration = 3200;
  }
...
function loadSessionForEditing(s, fallbackDayKey){
  ...
  // Load exactly what was saved, in exact order, with no automatic merging
  // against the current Program. Re-merging on every reopen was the source of
  // exercises quietly duplicating or reordering across repeated edit/save
  // cycles. Use "+ Add remaining exercises from today's plan" (below the
  // exercise list) to pull in anything from the Program not logged yet.
  logState.exercises = (s.exercises||[]).map(ex=>{
    const tmplMatch = (PLAN[logState.dayKey]?.exercises||[]).find(t=>t.name===ex.name);
    return {
      id: uid(),
      name: ex.name,
...
function ensureAddRemainingOverlay(){
  let ov = document.getElementById('addRemainingOverlay');
  if(ov) return ov;
  ov = document.createElement('div');
  ov.id = 'addRemainingOverlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);display:none;align-items:flex-end;justify-content:center;z-index:200;';
  document.body.appendChild(ov);
  return ov;
}
// Finds the most recent PAST session logged for a given Program day (e.g. the
// last time you actually did a "Wednesday" session), so you can pull in
// exactly what you did last time — including any swaps/extras — rather than
// starting from the generic Program template.
function getLastSessionForDay(dayKey, beforeDate){
  const matches = sessions
    .filter(s => s.dayKey === dayKey && s.date < beforeDate)
    .sort((a,b) => b.date.localeCompare(a.date));
  return matches[0] || null;
}
function copyLastWorkoutForDay(){
  const last = getLastSessionForDay(logState.dayKey, logState.date);
  if(!last) return null;
  const fullTemplate = emptyExercisesFor(logState.dayKey);
  logState.exercises = (last.exercises||[]).map(ex=>{
    const tmplMatch = fullTemplate.find(t=>t.name===ex.name);
    return {
      id: uid(),
      name: ex.name,
      target: tmplMatch ? tmplMatch.target : '',
      hero: tmplMatch ? tmplMatch.hero : !!(ex.tier==='hero'),
      custom: !tmplMatch,
      // Exercise structure only — empty sets, ready for today's actual numbers.
      // "Last time" reference on each card still shows what you did last time.
      sets: (ex.sets||[]).map(()=>({ id: uid(), weight:'', reps:'', rir:'', note:'' }))
    };
  });
  return last.date;
}
function openAddRemainingPicker(main){
  const ov = ensureAddRemainingOverlay();
  const fullTemplate = emptyExercisesFor(logState.dayKey);
  const selected = new Set();
  function render(){
    ov.innerHTML = `<div style="background:var(--surface);border-radius:16px 16px 0 0;padding:20px 18px 24px;width:100%;max-width:480px;max-height:82vh;overflow-y:auto;">
      <div style="font-family:var(--font-display);font-weight:600;font-size:16px;margin-bottom:4px;">Add from today's plan</div>
      <div style="font-size:12px;color:var(--text-dim);margin-bottom:14px;">Tick whichever ones you still need. Nothing gets added automatically — if you've renamed an exercise below (e.g. swapped equipment), just leave its original entry unticked.</div>
      ${fullTemplate.map((t,i)=>{
        const on = selected.has(i);
        return `<button class="addRemainingRow" data-idx="${i}" style="width:100%;display:flex;align-items:center;gap:10px;padding:9px 0;border-top:1px solid var(--border);border-left:none;border-right:none;border-bottom:none;background:none;text-align:left;color:var(--text);">
          <span class="checkbox" style="width:16px;height:16px;border-radius:4px;border:1px solid ${on?'var(--brass)':'var(--border)'};background:${on?'var(--brass)':'none'};display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;font-size:10px;color:${on?'#0A1F1A':'transparent'};">&#10003;</span>
          <span style="flex:1;font-size:13px;">${escapeHtml(t.name)}${t.target?` <span style="color:var(--text-faint);font-family:var(--font-mono);font-size:11px;">${escapeHtml(t.target)}</span>`:''}</span>
        </button>`;
      }).join('')}
      <button id="confirmAddRemainingBtn" style="width:100%;padding:13px;border-radius:9px;border:none;background:var(--brass);color:#0A1F1A;font-family:var(--font-display);font-weight:600;font-size:14px;margin-top:16px;">Add selected</button>
      <button id="cancelAddRemainingBtn" style="width:100%;padding:11px;border-radius:9px;border:1px solid var(--border);background:none;color:var(--text-faint);font-family:var(--font-mono);font-size:12px;margin-top:10px;">Cancel</button>
    </div>`;
    ov.querySelectorAll('.addRemainingRow').forEach(btn=>{
      btn.onclick = ()=>{
    ...
    document.getElementById('cancelAddRemainingBtn').onclick = ()=>{ ov.style.display='none'; };
    document.getElementById('confirmAddRemainingBtn').onclick = ()=>{
      if(selected.size===0){ showToast('Select at least one exercise'); return; }
      [...selected].forEach(idx=>{ logState.exercises.push(fullTemplate[idx]); });
      ov.style.display='none';
      showToast(`Added ${selected.size} exercise${selected.size!==1?'s':''}`);
      renderLog(main);
    };
  }
  render();
...
function parseImportText(text, startingYear){
  ...
  blocks.forEach(block=>{
    ...
    contentLines.forEach(line=>{
      ...
      const looksCardio = /(run|walk|hike|bike|cycle|cycling)/.test(low) && !/\d+\s*(kg|lb)/.test(low);
      if(looksCardio && !cardio){
        const dm = /(\d+(?:\.\d+)?)\s*km/.exec(low);
        const tm = /(\d{1,2}:\d{2})/.exec(line);
        const cardioType = low.includes('run') ? 'Run' : /(bike|cycle|cycling)/.test(low) ? 'Bike' : 'Walk';
        cardio = { active:true, type: cardioType, distance: dm?dm[1]:'', time: tm?tm[1]:'', note: line };
        return;
      }
...
function renderExerciseCleanup(main){
  ...
  <div style="font-size:12px;color:var(--text-dim);line-height:1.5;margin-bottom:14px;">
    Every distinct exercise name from your logged history, most-used first. This only touches your training log — your current Program is untouched, edit that separately if needed. <b>Merge</b> rewrites names permanently into one (use for typos/duplicates). <b>Link</b> keeps names separate but lets "last time"/deload/warm-up suggestions see across all of them — use this for exercises you rotate between (different machines, same slot).
  </div>`;
  if(list.length === 0){
  ...
  html += `<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border);">
    ...
    <button id="cleanupLinkBtn" style="width:100%;padding:11px;border-radius:9px;border:1px solid var(--steel);background:none;color:var(--steel);font-family:var(--font-display);font-weight:600;font-size:13px;">&#128279; Link selected (keep separate, share history)</button>
  </div>`;
  if(exerciseGroups.length){
    html += `<div class="section-label" style="margin-top:18px;">Linked exercise groups</div>`;
    html += exerciseGroups.map(g=>`
      <div class="progrow" style="margin-bottom:6px;align-items:flex-start;">
        <div style="font-size:12px;line-height:1.5;">${g.names.map(escapeHtml).join(' &#8596; ')}</div>
        <button class="unlinkGroupBtn" data-id="${g.id}" style="background:none;border:none;color:var(--rust);font-family:var(--font-mono);font-size:11px;flex-shrink:0;">UNLINK</button>
      </div>`).join('');
  }
  main.innerHTML = html;
  ...
  document.getElementById('cleanupLinkBtn').onclick = ()=>{
    if(cleanupSelected.size < 2){ showToast('Select at least two names to link together'); return; }
    const names = [...cleanupSelected];
    linkExerciseNames(names);
    showToast(`Linked ${names.length} exercise names — their history is now shared for recall`);
    cleanupSelected = new Set();
    renderExerciseCleanup(main);
  };
  main.querySelectorAll('.unlinkGroupBtn').forEach(btn=>{
    btn.onclick = ()=>{
      exerciseGroups = exerciseGroups.filter(g=>g.id!==btn.dataset.id);
      saveExerciseGroups();
      renderExerciseCleanup(main);
    };
  });
...
function renderReport(main){
  ...
  <div class="card" style="margin-bottom:14px;">
    ...
    ${reportPreset==='custom' ? `
    <div class="row2">
      <div class="field"><label>From</label><div style="overflow:hidden;border-radius:7px;"><input type="date" id="reportFromInput" value="${reportCustomFrom||range.from}" style="border-radius:7px;"></div></div>
      <div class="field"><label>To</label><div style="overflow:hidden;border-radius:7px;"><input type="date" id="reportToInput" value="${reportCustomTo||range.to}" style="border-radius:7px;"></div></div>
    </div>` : ''}
  ...
  heroGroups.forEach((label, key)=>{
    ...
    div.innerHTML = `
      <div class="liftheader">
        ...
        <div>
          <div class="liftvalue" style="color:${color};">${last!==undefined?last+'kg':'—'}</div>
          <div class="liftnote" style="text-align:right;">Latest${best!==null&&best!==last?` · 🏆 PR ${best}kg`:''}</div>
          ${delta!==null?`<div class="liftdelta" style="color:${delta>0?'var(--sage)':delta<0?'var(--rust)':'var(--text-faint)'};">${delta>0?'+':''}${delta}kg since ${formatAU(data[0].date)}</div>`:''}
        </div>
      </div>
  ...
  function renderResults(filter){
    ...
    resultsEl.querySelectorAll('.libExerciseBtn').forEach(b=>{
      b.onclick = ()=>{
        ...
        if(editorMode !== 'template') savePlan();
...
function renderTemplatesScreen(main){
  ...
  if(templates.length === 0){
    ...
    [...templates].sort((a,b)=>b.createdDate.localeCompare(a.createdDate)).forEach(t=>{
      html += `<div class="card">
        ...
        <div style="display:flex;gap:10px;">
          <button class="editTemplateBtn" data-id="${t.id}" style="flex:1;padding:9px;border-radius:8px;border:1px solid var(--brass);background:none;color:var(--brass);font-family:var(--font-mono);font-weight:600;font-size:12px;">Edit</button>
  ...
  main.querySelectorAll('.editTemplateBtn').forEach(btn=>{
    btn.onclick = ()=>{
      editorMode = 'template';
...
function renderProgram(main){
  ...
  DAY_ORDER.forEach(k=>{
    ...
    if(d.type==='lift'){
      ...
      sub = exList.length ? exList.map(e=>{
        ...
        const dw = computeDeloadWeightSuggestion(e.name);
        return `<div style="margin-top:3px;">${escapeHtml(e.name)}${e.target?` <span style="color:var(--text-faint);">${escapeHtml(e.target)}</span>`:''}${dl?` <span style="color:var(--steel);">· deload ${escapeHtml(dl)}${dw!==null?` (${fmtWeight(dw)})`:''}</span>`:''}</div>`;
  ...
  main.innerHTML = html;
  document.getElementById('editProgramBtn').onclick = ()=>{ editingProgram = true; editorMode = 'live'; editingTemplateId = null; renderProgramEditor(main); };
...
function renderProgramEditor(main){
  const plan = currentEditorPlan();
  const headerLabel = editorMode==='template'
    ? `Edit template: ${escapeHtml((templates.find(t=>t.id===editingTemplateId)||{}).name||'')}`
    : 'Edit program';
  let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;gap:8px;">
    <div class="section-label" style="margin:0;">${headerLabel}</div>
    <button id="doneEditingBtn" class="pill active" style="padding:6px 14px;flex-shrink:0;">Done</button>
  </div>`;
  DAY_ORDER.forEach(k=>{
    const d = plan[k];
    html += `<div class="card">
    ...
    if(d.type !== 'lift'){
      ...
      (d.exercises||[]).forEach((ex,i)=>{
        const dl = computeDeloadTarget(ex.target);
        const dw = computeDeloadWeightSuggestion(ex.name);
        const dlText = dl ? `Deload: ${dl}${dw!==null?` — try ${fmtWeight(dw)}`:''}` : '';
        html += `<div style="margin-bottom:6px;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:3px;margin-left:2px;">
            <div class="exDeloadPreview" data-day="${k}" data-exidx="${i}" style="font-size:10px;color:var(--steel);font-family:var(--font-mono);">${escapeHtml(dlText)}</div>
...
function wireProgramEditor(main){
  const plan = currentEditorPlan();
  document.getElementById('doneEditingBtn').onclick = ()=>finishEditingPlan(main);
  ...
  main.querySelectorAll('.exTargetEdit').forEach(inp=>{
    inp.oninput = e=>{
      ...
      if(preview){
        ...
        const exName = plan[inp.dataset.day].exercises[inp.dataset.exidx].name;
        const dw = computeDeloadWeightSuggestion(exName);
        preview.textContent = dl ? `Deload: ${dl}${dw!==null?` — try ${fmtWeight(dw)}`:''}` : '';
      }
    };
  });
...
function daysSinceLastBackup(){
  ...
  return Math.floor((new Date(todayISO()) - new Date(settings.lastBackupAt)) / 86400000);
}
function renderBackupReminder(){
  ...
  if(days === null){
    el.innerHTML = `<div style="background:var(--rust-dim);border:1px solid var(--rust-dim);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:12px;color:var(--rust);">You haven't downloaded a backup yet — worth doing now.</div>`;
  } else if(days >= 7){
    el.innerHTML = `<div style="background:var(--rust-dim);border:1px solid var(--rust-dim);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:12px;color:var(--rust);">It's been ${days} days since your last backup (${formatAU(settings.lastBackupAt)}) — worth downloading a fresh one.</div>`;
  } else {
    el.innerHTML = `<div style="font-size:11px;color:var(--text-faint);margin-bottom:12px;">Last backup: ${formatAU(settings.lastBackupAt)} (${days} day${days!==1?'s':''} ago)</div>`;
  }
}
...
document.getElementById('exportDataBtn').onclick = ()=>{
  const backup = {
    ...
    sessions, cycleEntries, settings, PLAN, goals, readinessEntries, measureEntries, templates, habits, exerciseGroups,
  };
  ...
  settings.lastBackupAt = todayISO();
  saveSettings();
  renderBackupReminder();
...
document.getElementById('importFileInput').onchange = (e)=>{
  ...
  reader.onload = ()=>{
    try{
      ...
      if(Array.isArray(backup.exerciseGroups)){
        backup.exerciseGroups.forEach(bg=>{
          if(bg.names && bg.names.length) linkExerciseNames(bg.names);
        });
      }
      document.getElementById('settingsOverlay').classList.remove('show');
      renderAll();
      const extras = [];
      if(backup.PLAN) extras.push('program');
      if(backup.settings) extras.push('settings');
      showToast(`Restored ${toAdd.length} session${toAdd.length!==1?'s':''}${extras.length?` + ${extras.join(' & ')}`:''} from backup`);
    }catch(err){
