...
function emptyExercisesFor(dayKey){
  ...
  return (day.exercises||[]).map(e => {
    ...
    return { id: uid(), name: e.name, target: e.target, hero: !!e.hero, custom:false, sets };
  });
}
...
function renderLog(main){
  ...
  if(addRemainingBtn) addRemainingBtn.onclick = ()=>{
    const added = addRemainingPlanExercises();
    if(added) showToast(`Added ${added} exercise${added!==1?'s':''} from today's plan`);
    renderLog(main);
  };
...
function wireExerciseCards(main){
  main.querySelectorAll('.ex-name-input').forEach(inp=>{
    inp.oninput = e=>{ const ex = logState.exercises.find(x=>x.id===inp.dataset.exId); if(ex){ ex.name = e.target.value; debouncedSaveDraft(); } };
  });
...
function addRemainingPlanExercises(){
  const fullTemplate = emptyExercisesFor(logState.dayKey);
  const existingKeys = new Set(logState.exercises.map(ex=>canonicalKey(ex.name)));
  const toAdd = fullTemplate.filter(t=>!existingKeys.has(canonicalKey(t.name)));
  if(toAdd.length === 0){ showToast("Everything from today's plan is already here"); return; }
  logState.exercises = [...logState.exercises, ...toAdd];
  return toAdd.length;
}
