...
function computeWeeklyAdherence(numWeeks){
  ...
  for(let w=numWeeks-1; w>=0; w--){
    ...
    for(let d=0; d<7; d++){
      ...
      if(planDay && planDay.type !== 'rest'){
        scheduled++;
        if(sessions.some(s=>s.date===iso)) logged++;
      }
    }
    weeks.push({ weekStart, scheduled, logged, pct: scheduled ? Math.round(logged/scheduled*100) : null });
  }
  return weeks;
}
function renderAdherenceBarChart(container, weeks){
  ...
  weeks.forEach((wk, i)=>{
    ...
    if(wk.pct !== null){
      bars += `<text x="${(x+barW/2).toFixed(1)}" y="${(y-4).toFixed(1)}" fill="#67655F" font-size="9" font-family="IBM Plex Mono, monospace" text-anchor="middle">${pct}%</text>`;
    }
    if(i===0 || i===weeks.length-1 || i===Math.floor(weeks.length/2)){
      labels += `<text x="${(x+barW/2).toFixed(1)}" y="${h-4}" fill="#67655F" font-size="9" font-family="IBM Plex Mono, monospace" text-anchor="middle">${formatAU(wk.weekStart).slice(0,5)}</text>`;
    }
  });
...
function renderProgress(main){
  ...
  renderAdherenceBarChart(document.getElementById('adherenceChart'), computeWeeklyAdherence(8));
}
