...
let sessions = [];
let goals = [];
let settings = { startDate: DEFAULT_START_DATE, completeAcknowledged: false, adaptiveDeloadStart: null };
let PLAN = {};
...
function renderReadinessCard(dateISO){
  ...
  const FIELDS = [
    {key:'sleep', label:'Sleep'}, {key:'energy', label:'Energy'},
    {key:'calm', label:'Calm'}, {key:'mood', label:'Mood'}
  ];
  const rows = FIELDS.map(f=>`
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
      <span style="font-size:11px;color:var(--text-faint);font-family:var(--font-mono);width:52px;flex-shrink:0;">${f.label.toUpperCase()}</span>
...
function renderDeloadBanner(dateISO){
  ...
  const programmedDeload = getPhaseForDate(dateISO).deload;
  if(recent && recent.daysLogged>=2 && recent.avg < ADAPTIVE_DELOAD_THRESHOLD && !programmedDeload){
...
function renderMeasure(main){
  ...
  document.getElementById('measureWaist').oninput = e=>{ d.waist = e.target.value; };
  document.getElementById('measureHips').oninput = e=>{ d.hips = e.target.value; };
  document.getElementById('measureChest').oninput = e=>{ d.chest = e.target.value; };
  document.getElementById('measureThigh').oninput = e=>{ d.thigh = e.target.value; };
  document.getElementById('measureArm').oninput = e=>{ d.arm = e.target.value; };
...
document.getElementById('exportDataBtn').onclick = ()=>{
  const backup = {
    ...
    sessions, cycleEntries, settings, PLAN, goals, readinessEntries, measureEntries,
  };
