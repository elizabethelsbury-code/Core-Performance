...
function renderExerciseCleanup(main){
  ...
  if(list.length === 0){
    ...
    html += `<div style="max-height:none;">` + list.map(item=>`
      <label style="display:flex;align-items:center;gap:10px;padding:9px 0;border-top:1px solid var(--border);">
        <input type="checkbox" class="cleanupCheck" data-name="${escapeAttr(item.name)}" ${cleanupSelected.has(item.name)?'checked':''} style="width:18px;height:18px;flex-shrink:0;">
  ...
  main.querySelectorAll('.cleanupCheck').forEach(cb=>{
    cb.onchange = ()=>{
      if(cb.checked) cleanupSelected.add(cb.dataset.name); else cleanupSelected.delete(cb.dataset.name);
