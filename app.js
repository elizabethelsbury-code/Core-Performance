...
function wireCycleView(main){
  ...
  document.getElementById('saveCycleBtn').onclick = ()=>{
    const anyDomain = Object.values(cycleState.domains).some(v=>v && v.length);
    const anyProm = Object.values(cycleState.proms).some(v=>v!=='' && v!=null);
    const hasContent = cycleState.flow!=='None' || cycleState.periodStart || cycleState.mSymptoms.length || anyDomain ||
      Object.values(cycleState.nrs).some(v=>v!=null) || cycleState.provocation.trim() || cycleState.breath.length ||
      anyProm || cycleState.note.trim() || Object.values(cycleState.timing).some(v=>v) || cycleState.settling || cycleState.tolerance || cycleState.followUp24h;
...
function renderLog(main){
  ...
  html += `<div class="field" style="margin-bottom:14px;"><label>Warm-up</label><input type="text" id="logWarmup" autocomplete="off" placeholder="e.g. 6 min incline walk" value="${escapeAttr(logState.warmup)}"></div>`;
  ...
  if(logState.cardio.active){
    html += `<div class="card">
      ...
      <div class="row2">
        <div class="field"><label>Distance (km)</label><input type="text" inputmode="decimal" id="cardioDist" autocomplete="off" value="${escapeAttr(logState.cardio.distance)}" placeholder="5.06"></div>
        <div class="field"><label>Time (mm:ss)</label><input type="text" id="cardioTime" autocomplete="off" value="${escapeAttr(logState.cardio.time)}" placeholder="25:13"></div>
      </div>
