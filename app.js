...
function ensureRestTimerBar(){
  ...
  bar.id = 'restTimerBar';
  bar.style.cssText = 'position:fixed;left:0;right:0;top:0;background:var(--surface);border-bottom:1px solid var(--border);padding:calc(max(10px, env(safe-area-inset-top)) + 6px) 14px 10px;display:none;z-index:250;';
...
function renderRestTimerBar(){
  ...
  if(state){
    bar.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
      <button id="restMinusBtn" style="background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:8px 12px;font-family:var(--font-mono);font-size:13px;">-15</button>
      <div style="flex:1;text-align:center;">
        <div id="restTimeDisplay" style="font-family:var(--font-display);font-weight:600;font-size:22px;color:var(--brass);"></div>
        <div style="font-size:10px;color:var(--text-faint);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:0.04em;">Rest</div>
      </div>
      <button id="restPlusBtn" style="background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:8px 12px;font-family:var(--font-mono);font-size:13px;">+15</button>
      <button id="restStopBtn" style="background:none;border:none;color:var(--rust);font-size:18px;padding:4px 8px;">&times;</button>
    </div>`;
    ...
    if(restTimerInterval){ clearInterval(restTimerInterval); restTimerInterval = null; }
    bar.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">
      <span style="font-size:10px;color:var(--text-faint);font-family:var(--font-mono);text-transform:uppercase;letter-spacing:0.04em;margin-right:2px;">Rest</span>
      ${[60,90,120,180].map(s=>`<button class="restPresetBtn" data-s="${s}" style="background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:6px 12px;font-family:var(--font-mono);font-size:12px;">${s}s</button>`).join('')}
    </div>`;
...
// resume an in-progress rest timer if the app was reloaded/backgrounded mid-rest,
// and keep the bar in sync with every render (shown only on the Log view)
const _origRenderAllForRest = renderAll;
renderAll = function(){ _origRenderAllForRest(); renderRestTimerBar(); };
