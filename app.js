...
function loadCycleStateForDate(dateStr){
  ...
  if(found){
    const domainsCopy = {};
    Object.keys(found.domains||{}).forEach(k=>{ domainsCopy[k] = [...(found.domains[k]||[])]; });
    cycleState = {
      ...
      domains: domainsCopy, nrs: {...(found.nrs||{})}, timing: {...(found.timing||{})},
...
function coreExerciseName(name){
  return (name||'').toLowerCase()
    .replace(/\b(barbell|bb|db|dumbbell|banded|cable|machine|smith|conventional|weighted)\b/g, '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
}
function getLastLoggedExercise(name){
  ...
  const core = coreExerciseName(name);
  if(!core) return null;
  for(const s of sorted){
    const ex = (s.exercises||[]).find(e=>{
      const c = coreExerciseName(e.name);
      return c && c.length > 2 && (c === core || c.includes(core) || core.includes(c));
    });
    if(ex && ex.sets && ex.sets.length){
      return { date: s.date, sets: ex.sets };
    }
...
function renderBigLifts(main){
  ...
  BIG_LIFT_DEFS.forEach(lift=>{
    ...
    renderLineChart(div.querySelector('.chart-slot'), data, lift.color, lift.color.startsWith('var')?'#4CA893':lift.color, true);
  });
}
